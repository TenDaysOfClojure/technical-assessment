...
(defn start-server! [& {:keys [port] :as options}]
  (if (not (http-server-running? port))
    (do
      (logging/info "Starting HTTP server with options" options)
    ...
    (logging/warn "HTTP server already running with options" options)))
  ...
  (if-let [config (get-http-server port)]
    ...
    (when (:running? config)
      (let [current-server (:server config)]
        ...
        (logging/info "HTTP server stopped on port" port)))

    ;; No server running, not stopping.
    (logging/warn "No HTTP server running on port" port " to stop")))
